 ...
 */
function draftly_customize_register( $wp_customize ) {
	$wp_customize->get_setting( 'blogname' )->transport         = 'postMessage';
	$wp_customize->get_setting( 'blogdescription' )->transport  = 'postMessage';
	$wp_customize->get_setting( 'header_textcolor' )->transport = 'postMessage';
	$wp_customize->get_section('header_image')->title = __( 'Header Settings', 'draftly' );

	if ( isset( $wp_customize->selective_refresh ) ) {
		$wp_customize->selective_refresh->add_partial( 'blogname', array(
			'selector'        => '.site-title a',
			'render_callback' => 'draftly_customize_partial_blogname',
		) );
		$wp_customize->selective_refresh->add_partial( 'blogdescription', array(
			'selector'        => '.site-description',
			'render_callback' => 'draftly_customize_partial_blogdescription',
		) );
	}
	...
	$wp_customize->selective_refresh->add_partial(
		...
		array(
			...
			'render_callback' => 'draftly_customize_partial_site_logo',
		)
	);
	...
	/* New Section */
	$wp_customize->add_setting( 'hide_header_text', array(
		'default' => 0,
		'sanitize_callback' => 'draftly_sanitize_checkbox',
	) );

	$wp_customize->add_control( 'hide_header_text', array(
		'label'    => __( 'Hide Header Text', 'draftly' ),
		'section'  => 'header_image',
		'priority' => 1,
		'settings' => 'hide_header_text',
		'type'     => 'checkbox',
	) );
	...
	$wp_customize->add_section( 'sidebar_settings', array(
		'title'      => __('Sidebar Settings','draftly'),
		'priority'   => 1,
	...
	function draftly_sanitize_checkbox( $input ) {
...
add_action( 'customize_register', 'draftly_customize_register' );




if(! function_exists('draftly_customizer_css_final_output' ) ):
	function draftly_customizer_css_final_output(){
		?>
	...
	<?php }
	add_action( 'wp_head', 'draftly_customizer_css_final_output' );
endif;
 ...
 * @return void
 */
function draftly_customize_partial_blogname() {
	bloginfo( 'name' );
}
 ...
 * @return void
 */
function draftly_customize_partial_blogdescription() {
	bloginfo( 'description' );
}

if ( ! function_exists( 'draftly_customize_partial_site_logo' ) ) {
	/**
	 * Render the site logo for the selective refresh partial.
	 *
	 * Doing it this way so we don't have issues with `render_callback`'s arguments.
	 */
	function draftly_customize_partial_site_logo() {
		the_custom_logo();
	}
}
 ...
 */
function draftly_customize_preview_js() {
	wp_enqueue_script( 'draftly-customizer', get_template_directory_uri() . '/js/customizer.js', array( 'customize-preview' ), '20151215', true );
}
add_action( 'customize_preview_init', 'draftly_customize_preview_js' );
